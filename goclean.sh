...
test -z "$(gometalinter --disable-all --enable=golint $linter_targets 2>&1 | egrep -v '(ALL_CAPS|OP_|NewFieldVal|RpcCommand|RpcRawCommand|RpcSend|Dns|api.pb.go|StartConsensusRpc|factory_test.go|legacy|UnstableAPI)' | tee /dev/stderr)"
test -z "$(gometalinter --disable-all --enable=vet $linter_targets 2>&1 | egrep -v 'not a string in call to [A-Za-z]+f' | tee /dev/stderr)"

env GORACE="halt_on_error=1" go test -race -short $linter_targets
