...
func (w *Wallet) newCsppJoin(change *wire.TxOut, amount dcrutil.Amount, mixAccount, mixBranch uint32, mcount int) *csppJoin {
	cj := &csppJoin{
...
func (c *csppJoin) Gen() ([][]byte, error) {
	...
	for i := 0; i < c.mcount; i++ {
		persist := c.wallet.deferPersistReturnedChild(&updates)
		mixAddr, err := c.wallet.nextAddress(context.TODO(), op, persist, c.mixAccount, c.mixBranch, WithGapPolicyIgnore())
		if err != nil {
			return nil, err
		}
	...
	err := walletdb.Update(c.wallet.db, func(dbtx walletdb.ReadWriteTx) error {
...
func (c *csppJoin) Confirm() error {
	...
	err := walletdb.View(c.wallet.db, func(dbtx walletdb.ReadTx) error {
