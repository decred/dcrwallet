...
func TestDualCoinTxRules(t *testing.T) {
	...
	varOutputs := []*wire.TxOut{
		{Value: 100000000, CoinType: wire.CoinType(dcrutil.CoinTypeVAR)}, // 1 VAR
	}
	...
	skaOutputs := []*wire.TxOut{
		{Value: 200000000, CoinType: wire.CoinType(dcrutil.CoinType(1))}, // 2 SKA-1
	}
	
	coinType = txrules.GetPrimaryCoinTypeFromOutputs(skaOutputs)
	if coinType != dcrutil.CoinType(1) {
	...
	mixedOutputs := []*wire.TxOut{
		{Value: 100000000, CoinType: wire.CoinType(dcrutil.CoinTypeVAR)}, // 1 VAR
		{Value: 200000000, CoinType: wire.CoinType(dcrutil.CoinType(2))}, // 2 SKA-2
	}
	
	coinType = txrules.GetPrimaryCoinTypeFromOutputs(mixedOutputs)
	if coinType != dcrutil.CoinType(2) {
...
func TestDualCoinFeeCalculation(t *testing.T) {
	...
	varFee := txrules.FeeForSerializeSizeDualCoin(relayFeePerKb, txSize, dcrutil.CoinTypeVAR)
	...
	skaFee := txrules.FeeForSerializeSizeDualCoin(relayFeePerKb, txSize, dcrutil.CoinType(1))
...
func TestTxAuthorFeeHandling(t *testing.T) {
	...
	varOutputs := []*wire.TxOut{
		{Value: 100000000, CoinType: wire.CoinType(dcrutil.CoinTypeVAR)},
	}
	...
	skaOutputs := []*wire.TxOut{
		{Value: 100000000, CoinType: wire.CoinType(dcrutil.CoinType(1))},
	}
