...
func setupWallet(t *testing.T, cfg *Config) (w *Wallet, db walletdb.DB, teardown func()) {
...
func testExternalAddresses(tc *testContext) {
	...
	for i := 0; i < len(expectedExternalAddrs); i++ {
		...
		if maddr.Compressed() != expectedExternalAddrs[i].compressed {
			tc.t.Fatalf("%s: expected compressed status of %v got %v", prefix,
				expectedExternalAddrs[i].compressed, maddr.Compressed())
		}
...
func testInternalAddresses(tc *testContext) {
	...
	for i := 0; i < len(expectedInternalAddrs); i++ {
		...
		if maddr.Compressed() != expectedInternalAddrs[i].compressed {
			tc.t.Fatalf("%s: expected compressed status of %v got %v", prefix,
				expectedExternalAddrs[i].compressed, maddr.Compressed())
		}
