...
func fetchMinedBalance(ns walletdb.ReadBucket) (exccutil.Amount, error) {
	...
	return exccutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt exccutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (exccutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (exccutil.Amount, bool, error) {
	if len(v) < 9 {
	...
	return exccutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (exccutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read for raw credit amount change "+
			"(expected %d bytes, read %d)", bucketCredits, 9, len(v))
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	return exccutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (exccutil.Amount, error) {
	...
	return exccutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (exccutil.Amount, error) {
	...
	return exccutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = exccutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount exccutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount exccutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) exccutil.Amount {
	return exccutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnminedCredit(amount exccutil.Amount, change bool, opCode uint8,
...
func fetchRawUnminedCreditAmount(v []byte) (exccutil.Amount, error) {
...
func fetchRawUnminedCreditAmountChange(v []byte) (exccutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		str := "short unmined credit value when look up credit amt change"
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	amt := exccutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
...
func keyTxScript(script []byte) []byte {
	return exccutil.Hash160(script)
}
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount exccutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = exccutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) exccutil.Amount {
	return exccutil.Amount(byteOrder.Uint64(v[59:67]))
}
