...
// Helper function to create test transaction outputs with specific coin types
func createTestTxOut(value int64, coinType dcrutil.CoinType) *wire.TxOut {
	return &wire.TxOut{
		Value:    value,
		Version:  0,
		PkScript: make([]byte, 25), // P2PKH size
		CoinType: wire.CoinType(coinType),
	}
}

// Helper function to create test transaction outputs as TransactionOutput structs
func createTestTransactionOutput(value int64, coinType dcrutil.CoinType, height int32) *TransactionOutput {
...
func TestOutputSelectionPolicyCoinTypeFilter(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			policyFilter:   nil,
			outputCoinType: dcrutil.CoinTypeVAR,
			shouldInclude:  true,
		...
		},
		{
			...
			policyFilter:   nil,
			outputCoinType: dcrutil.CoinType(1),
			shouldInclude:  true,
			description:    "No filter should include all coin types",
		},
		{
			name:           "VAR filter - VAR output",
			policyFilter:   func() *dcrutil.CoinType { ct := dcrutil.CoinTypeVAR; return &ct }(),
			outputCoinType: dcrutil.CoinTypeVAR,
			shouldInclude:  true,
		...
		},
		{
			...
			policyFilter:   func() *dcrutil.CoinType { ct := dcrutil.CoinType(1); return &ct }(),
			outputCoinType: dcrutil.CoinType(1),
			shouldInclude:  true,
		...
		},
		{
			...
			policyFilter:   func() *dcrutil.CoinType { ct := dcrutil.CoinType(1); return &ct }(),
			outputCoinType: dcrutil.CoinType(2),
			shouldInclude:  false,
...
func filterOutputsByCoinType(outputs []*TransactionOutput, coinType *dcrutil.CoinType) []*TransactionOutput {
	if coinType == nil {
		return outputs
	}
	...
	for _, output := range outputs {
		if dcrutil.CoinType(output.Output.CoinType) == *coinType {
...
func TestInputStructCoinType(t *testing.T) {
	testCases := []struct {
		...
		{"VAR input", dcrutil.CoinTypeVAR, 1e8},
		{"SKA-1 input", dcrutil.CoinType(1), 2e8},
		{"SKA-255 input", dcrutil.CoinType(255), 3e8},
	}
...
func detectMixedCoinTypes(coinTypes []dcrutil.CoinType) bool {
	if len(coinTypes) <= 1 {
		return false
	}
...
func selectUTXOsByCoinType(utxos []struct {
	value    dcrutil.Amount
	coinType dcrutil.CoinType
}, target dcrutil.Amount, coinType dcrutil.CoinType) []struct {
	value    dcrutil.Amount
	coinType dcrutil.CoinType
} {
	var selected []struct {
