...
func TestBalancesStructureMultiCoin(t *testing.T) {
	...
	// Add VAR balance
	balance.CoinTypeBalances[dcrutil.CoinTypeVAR] = udb.CoinBalance{
		CoinType:  dcrutil.CoinTypeVAR,
		Spendable: 1e8,
		Total:     1e8,
	}
	...
	// Add SKA-1 balance
	balance.CoinTypeBalances[dcrutil.CoinType(1)] = udb.CoinBalance{
		CoinType:  dcrutil.CoinType(1),
		Spendable: 5e7,
		Total:     5e7,
	}
	...
	varBalance, exists := balance.CoinTypeBalances[dcrutil.CoinTypeVAR]
	if !exists {
	...
	ska1Balance, exists := balance.CoinTypeBalances[dcrutil.CoinType(1)]
	if !exists {
...
func TestAccountBalanceNotificationStructure(t *testing.T) {
	accountBalance := AccountBalance{
		...
		CoinTypeBalances: map[dcrutil.CoinType]dcrutil.Amount{
			dcrutil.CoinTypeVAR: 1e8,
			dcrutil.CoinType(1): 5e7,
			dcrutil.CoinType(2): 2e6,
		},
	}
	...
	expectedBalances := map[dcrutil.CoinType]dcrutil.Amount{
		dcrutil.CoinTypeVAR: 1e8,
		dcrutil.CoinType(1): 5e7,
		dcrutil.CoinType(2): 2e6,
	}
...
func TestMultiCoinBalanceMapFlattening(t *testing.T) {
	...
	multiCoinBalanceMap := map[uint32]map[dcrutil.CoinType]dcrutil.Amount{
		0: {
			dcrutil.CoinTypeVAR: 1e8,
			dcrutil.CoinType(1): 5e7,
		},
		1: {
			dcrutil.CoinTypeVAR: 2e8,
			dcrutil.CoinType(2): 3e7,
		},
	}
...
func TestCoinTypeIsolationInBalances(t *testing.T) {
	...
	ska1Balance := balance.CoinTypeBalances[dcrutil.CoinType(1)]
	ska255Balance := balance.CoinTypeBalances[dcrutil.CoinType(255)]
...
func TestUnminedCreditCoinTypeDefaulting(t *testing.T) {
	...
	if expectedCoinType != dcrutil.CoinTypeVAR {
		t.Errorf("Expected unmined credit default coin type to be VAR (%v), got %v",
			dcrutil.CoinTypeVAR, expectedCoinType)
	}
}
